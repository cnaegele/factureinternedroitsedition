...
import { getEmployesListe, sauveDroitEdition, getEmployesEdition } from '@/axioscalls.ts'
...
const employeEditionAjout = async (employeId: number) => {
    ...
    if (lesDatas.idService > 0) {
        ...
        if (!employeDejaChoisi) {
            ...
            const response: ApiResponseMessage = await sauveDroitEdition(lesDatas.idService, employeId)
